	...
	for requestDomainOrig := range onto.individuals {
		...
		for {
			...
			for i := 0; i < len(rr.Data); i++ {
				...
				/*
					if requestDomainOrig == "owl:Ontology" {
						log.Printf("attr %s\n", attr)
					}*/
				if attr == "v_s_permissionSubject" {
					log.Printf("range [%s]", rdfsRange)
				}
				switch rdfsRange {
				case "xsd:string", "rdfs:Literal":
					...
					wr.WriteString(fmt.Sprintf("\trt_attr_json = %s_attr\n", attr))
					wr.WriteString(fmt.Sprintf("\trt_attr_json = %s_attr_ru\n", attr))
					wr.WriteString(fmt.Sprintf("\trt_attr_json = %s_attr_en\n", attr))
				case "xsd:dateTime":
					domainAttrs = append(domainAttrs,
						ClassAttr{name: domain.Uri, fieldName: attr, attrName: attr + "_attr"})
					wr.WriteString(fmt.Sprintf("\trt_field = %s\n", attr))
