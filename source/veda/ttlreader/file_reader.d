...
//	int checktime = 30;
    ...
    if (need_remove_ontology)
    {
        ...
        foreach (uri; uris)
        {
            ...
            context.remove_individual(&sticket, uri, true, "ttl-reader", null, true, false);
        }
        ...
        {
            log.tracec("WARN: [%s] WILL BE REMOVED", uri);
            res = context.remove_individual(&sticket, uri, true, "ttl-reader", null, true, false);
        }
        ...
        for (int priority = 0; priority < 100; priority++)
        {
            ...
            foreach (uri, indv; individuals)
            {
				...
				string bin = indv.serialize ();
                                ...
                                ResultCode res = context.put_individual(&sticket, indv.uri, indv, true, null, null, false, false).result;
                                if (trace_msg[ 33 ] == 1)
...
//            catch (Exception ex) {}
        ...
        OpResult orc = context.put_individual(&sticket, indv_ttl_file.uri, indv_ttl_file, true, null, null, false, false);
